...
class CI_Pagination {
	...
	public function create_links()
	{
		...
		if ($this->prev_link !== FALSE && $this->cur_page !== 1)
		{
			...
			{
				$output .= $this->prev_tag_open.'<a  class="page-link"  href="'.$this->first_url.$query_string.'"'.$attributes.$this->_attr_rel('prev').'>'
					.$this->prev_link.'</a>'.$this->prev_tag_close;
			}
			else
			{
				$append = ($i === $base_page) ? $query_string : $this->prefix.$i.$this->suffix;
				$output .= $this->prev_tag_open.'<a  class="page-link"  ref="'.$this->base_url.$append.'"'.$attributes.$this->_attr_rel('prev').'>'
					.$this->prev_link.'</a>'.$this->prev_tag_close;
			}
			...
			for ($loop = $start -1; $loop <= $end; $loop++)
			{
				...
				if ($i >= $base_page)
				{
					...
					else
					{
						...
						{
							$output .= $this->num_tag_open.'<a class="page-link" href="'.$this->first_url.$query_string.'"'.$attributes.$this->_attr_rel('start').'>'
								.$loop.'</a>'.$this->num_tag_close;
						}
						else
						{
							$append = ($n === '') ? $query_string : $this->prefix.$n ;
							$output .= $this->num_tag_open.'<a class="page-link" href="'.$this->base_url.$append.'"'.$attributes.$this->_attr_rel('start').'>'
								.$loop.'</a>'.$this->num_tag_close;
						}
					}
				}
		...
		if ($this->next_link !== FALSE && $this->cur_page < $num_pages)
		{
			...
			$output .= $this->next_tag_open.'<a class="page-link" href="'.$this->base_url.$this->prefix.$i.$this->suffix.'"'.$attributes
		...
		if ($this->last_link !== FALSE && ($this->cur_page + $this->num_links) < $num_pages)
		{
			...
			$output .= $this->last_tag_open.'<a class="page-link" href="'.$this->base_url.$this->prefix.$i.$this->suffix.'"'.$attributes.'>'
	...
	public function create_links_admin()
	{
		// If our item count or per-page total is zero there is no need to continue.
		if ($this->total_rows === 0 OR $this->per_page === 0)
		{
		...
		if ($this->prev_link !== FALSE && $this->cur_page !== 1)
		{
			...
			{
				$append = ($i === $base_page) ? $query_string : $this->prefix.$i.$this->suffix;
				$output .= $this->prev_tag_open.'<a class="page-link" href="'.$this->base_url.$append.'"'.$attributes.$this->_attr_rel('prev').'>'
					.$this->prev_link.'</a>'.$this->prev_tag_close;
			}
