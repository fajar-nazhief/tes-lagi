...
class Widget_Latest_news extends Widgets
{
	...
	public function run($options)
	{
		...
		$this->db->select('count(id) as jml,intro_en');
		$this->db->group_by('intro_en');
		$this->db->order_by('jml','DESC');
		$this->db->limit($options['limit']);
		$this->db->where('status', 'live');
		$this->db->where('intro_en <>', ''); 
		$query = $this->db->get('news')->result();

		$news_widget =  $query;
		//$news_widget = $this->db->count_all_results('news');//$this->news_m->limit($options['limit'])->get_many_by(array('status' => 'live'));
