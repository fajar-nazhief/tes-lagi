...
class My_m extends MY_Model {
	...
	function get_all()
	{   
		$this->db->select($this->_table.'.*, Gantirelasi.title AS category_title, Gantirelasi.slug AS category_slug');
		$this->db->join('Gantirelasi', $this->_table.'.category_id = Gantirelasi.id', 'left');
	...
	function get_many_by($params = array())
	{
		...
		if (!empty($params['category']))
		{
			if (is_numeric($params['category']))
				$this->db->where($this->_table.'_categories.id', $params['category']);
			else
				$this->db->where($this->_table.'_categories.slug', $params['category']);
		}
	...
	function count_search($data = array())
	{
		  if (!empty($data['category']))
		{
			if (is_numeric($data['category'])){
				$this->db->where($this->_table.'.category_id', $data['category']);
			}
		...
		{
			$this->db->like($this->_table.'.json_data', $data['keywords']);
		}
	...
	function get_archive_months()
	{
		...
		$this->db->distinct();
		$this->db->select('(SELECT count(id) FROM ' . $this->db->dbprefix($this->_table) . ' t2
	...
	function get_blog_fragment($params = array())
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			{
				$string .= '<p>' . anchor($this->_table.'/' . date('Y/m') . '/' . $blog->slug, $blog->title) . '<br />' . strip_tags($blog->intro) . '</p>';
			}
		}
		return $string;
	}
	...
	public function search($data = array())
	{
		...
		/*if (array_key_exists('keywords', $data))
		{
			...
			foreach ($phrases as $phrase)
			{
				...
				//$this->db->or_like($this->_table.'.body', $phrase);
				//$this->db->or_like($this->_table.'.intro', $phrase);
				$counter++;
			}
		}
		*/
