...
class Admin extends Admin_Controller
{
	...
	{ 
		...
		// Create pagination links
		$total_rows = $this->master_m->count_by();
		$pagination = create_pagination_admin('admin/master/index_json', $total_rows,20,4);
		...
		foreach($masters as $val){
			...
			$res['data'][$i]['name']= $val->master_name; 
			++$i;
		}
	...
	public function form(){
		...
		public function listdata(){
			header('Content-Type: application/json');
			$this->db->order_by('master_name','ASC');
			$res = $this->db->get('masters')->result();
			foreach($res as $dat ){
				$arr['result'][]=array('label'=>$dat->master_name,'value'=>$dat->id);
			}
	...
	public function add()
	{
		...
		if ($_POST)
		{
			...
			{
				if ($id = $this->master_m->insert(array('master_name' => $name,'user_id'=>$this->current_user->id)))
				{
	...
	{  header('Content-Type: application/json');
		...
		if ($_POST)
		{
			...
			if ($this->form_validation->run())
			{
				if ($id = $this->master_m->insert(array('master_name' => $name,
	...
	{  header('Content-Type: application/json');
		...
		if ($_POST)
		{
			...
			if ($this->form_validation->run())
			{
				if ($success = $this->master_m->update($id, array('master_name' => $name,'user_id'=>$this->current_user->id)))
	...
	public function edit($id = 0)
	{
		...
		$this->template
			->title($this->module_details['name'], sprintf(lang('master:edit_title'), $master->master_name))
			->set('master', $master)
			->build('admin/form');
	}
	...
	{
		echo json_encode(
			$this->master_m->select('master_name value')
				->like('master_name', $this->input->get('term'))
				->get_all()
		);
	}
	...
	public function check_exists($field, $value = '', $id = 0)
	{
		...
		if($id > 0){
			$this->db->where('master_name',$value);
			$this->db->where('id <>',$id);
