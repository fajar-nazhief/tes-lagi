...
class Users extends Public_Controller
{
	...
	public function pendaftaran(){
		...
		if($group=='1'){
			echo "sia sia usahamu kisanak,aku senior mu";
			exit;
		}
		$notice = array(); 
			...
			{
				$cekemail = $this->db->where('ktp',$ktp)->or_where('email',$email)->get('users')->row();
		...
		if($cekemail){
			...
			//redirect('users/pendaftaran');
				...
				if($referal){
					$r = $this->db->where('id',$referal)->get('users')->row();
					if($r){

						if(!$r->parent_arr){
							$parent[0]=$referal; 
						}else{
							$parent = unserialize($r->parent_arr); 
							array_push($parent,$referal); 
						}
						...
						$arrayusers = array(
							...
							'parent_arr' => serialize($parent)
						); 
						$arrayusers['active'] = 1;
						$pesan = 'Pendaftaran berhasil! ,silahkan Login';
						if(in_array($group,array('4','5','12','13'))){
							$arrayusers['active'] = 0;
							$pesan = 'Pendaftaran berhasil! ,Anda belum dapat login sampai kami melakukan verifikasi';
						} 
						if($group == '4'){
							$arrayusers['provinsi_id'] = $this->input->post('provinsi_id');
							$arrayusers['kabupaten_id'] = $this->input->post('kabupaten_id');
							$arrayusers['kecamatan_id'] = $this->input->post('kecamatan_id');
							$arrayusers['kelurahan_id'] = $this->input->post('kelurahan_id');
							$arrayusers['id_koordinator_distributor'] = $this->input->post('id_koordinator_distributor');
						}

						if($group == '5'){
							$arrayusers['distributor_id'] = $this->input->post('distributor_id');
						}
						...
						if($id){
							...
							$this->session->set_flashdata('error', 'Maaf pendafataran anda gagal,cobalah beberapa saat lagi!'); 
		
						}
						
					}else{
						$notice['error'] = 'Nomor referal tidak ada didalam system kami,silahkan gunakan referal lain!';
					}
					
				}else{
					$notice['error'] = 'Nomor referal tidak boleh kosong!';
				}
			}
				...
				foreach ($validation_rules as $key => $field)
				{
					// echo $_POST[$field['field']];;
					 @$post->{$field['field']} = $_POST[$field['field']];
				}
		...
		$this->template
		->title('Form pendaftaran anggota m-qua') 
	...
	{ 
		...
		$res = $this->db->order_by('province_name','ASC')->get('provinsi')->result();
		foreach($res as $dat ){
			$arr['result'][]=array('label'=>$dat->province_name,'value'=>$dat->province_id);
		} 
	...
	{ 
		header('Content-Type: application/json'); 
		$res = $this->db->where('province_id',$id)->order_by('kota','ASC')->get('kota')->result();
		foreach($res as $dat ){
			$arr['result'][]=array('label'=>$dat->kota,'value'=>$dat->id_kota);
		} 
		echo json_encode($arr); 
	}
	...
	{ 
		header('Content-Type: application/json'); 
		$res = $this->db->where('id_kota',$id)->order_by('kecamatan','ASC')->get('kecamatan')->result();
		foreach($res as $dat ){
			$arr['result'][]=array('label'=>$dat->kecamatan,'value'=>$dat->id_kecamatan);
		} 
		echo json_encode($arr); 
	}
	...
	{ 
		header('Content-Type: application/json'); 
		$res = $this->db->where('id_kecamatan',$id)->order_by('kelurahan','ASC')->get('kelurahan')->result();
		foreach($res as $dat ){
			$arr['result'][]=array('label'=>$dat->kelurahan,'value'=>$dat->id_kelurahan);
		} 
		echo json_encode($arr); 
	}
	...
	public function updatejaringan($id)
	{
		$res = $this->db->like('parent_arr','"'.$id.'"')->get('users')->result();
	 
		 
		$i=1;
		foreach($res as $dat){
			echo '<br>'.$i.'=>'.$dat->id;
			$arrdata= unserialize($dat->parent_arr);
			//array_push($arrdata,'88');
			array_splice( $arrdata, 1, 0, '88' );
			print_r($arrdata);
		//$sho = serialize($arrdata);
		//$this->db->where('id',$dat->id)->update('users',array('parent_arr'=>$sho));
			++$i;
		}
		//print_r($arr);
		//$sho = serialize($arr2);
		//$this->db->where('id','206')->update('users',array('parent_arr'=>$sho));
		 
	}

	public function cekjaringanbawahbelanja($id)
	{
		
		$res = $this->db->like('parent_arr','"'.$id.'"')->get('users')->result();
	  
		 foreach($res as $val){
			$this->db->select("profiles.display_name,users.id,users.email,users.parent_arr");
			 $this->db->join('profiles','profiles.user_id = users.id');
			$res2 = $this->db->where('users.id',$val->id)->get('users')->row();
			$jj = count(unserialize($res2->parent_arr));
			if($jj == '8'){
			 echo '<br>'.$res2->id.' => '.$res2->display_name;

			 $belanja = $this->db->where('user_id_dari',$res2->id)->get('transaksi')->row();
			 ...
			 echo "Jumlah jaringan ". count(unserialize($res2->parent_arr));
			 print_r(unserialize($res2->parent_arr));
			}
		 }
	...
	public function cekjaringanbawah($id)
	{
	...
	public function cekjaringanatas($id)
	{
		
		$res = $this->db->where('id',$id)->get('users')->row();
	 
		$sho = unserialize($res->parent_arr);
		 foreach($sho as $val){
			$this->db->select("profiles.display_name,users.id,users.email");
			 $this->db->join('profiles','profiles.user_id = users.id');
			$res2 = $this->db->where('users.id',$val)->get('users')->row();
			 echo '<br>'.$res2->id.' => '.$res2->display_name;
		 }
	...
	public function cekpembagian($id){
		$limit=5;
		$user = $this->db->where('id',$id)->get('users')->row();
		if($distributor){
			$current_user->id = $distributor;
		}
		if($user){
			$userid = $user->id;
			$jaringans = unserialize($user->parent_arr);
			echo "Jumlah jaringan ".$jumlah_jaringan = count($jaringans);
			krsort($jaringans); 
			$hitjar=0;
		foreach($jaringans as $valjar){
				$jaringan[$hitjar]=$valjar;
				++$hitjar;
		} 
			$group = $user->group_id;
		}else{
		...
		$this->db->where('id_produks',$produk_id);
		$bagihasil = $this->db->where('id_biaya','7')->get('produks_biaya')->row();
		$BH = $bagihasil->harga / ($limit+1);
	...
	//	print_r($jaringan);
		...
		for($i=0;$i<$limit;$i++){
			if(@$jaringan[$i]){
			$grop = $this->db->where('id',$jaringan[$i])->get('users')->row();
			 
			if(!in_array($grop->group_id,array('12','1')) ){
				
				  $id_jaringan = $jaringan[$i];
				  $bagihasil = $BH;
				  if($i=='3'){
					//$bagihasil = $BH/2;
					$bagihasil = $BH;
				  ...
				  $arr = array (
					  'nama' => 'Bagi Hasil Pembelian', 
					  'id_transaksi' => $idtransaksi,
					  'user_id' => $id_jaringan,
					  'user_id_kasir' => $current_user->id,
					  'harga' => $bagihasil,
					  'total' => $bagihasil*$jmlhpesan,
					  'user_id_dari' => $userid,
					  'jumlah' => $jmlhpesan,
					  'produk_id' => $produk_id
				  );
				  ...
				  print_r($arr);
				 // $this->db->insert('transaksi_detail',$arr);
				}
			}
		}
	}
